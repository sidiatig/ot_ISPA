...
# values of pixels in active zones are set to be 1
# four experiments:
...
def main():
    ...
    note = "artificial_{}_{}noise_{}subs_{}samples_{}zones" \
    ...
    if loc_type == 'fixed':  # fixed, 2 subs, one zone
        for sub_id in range(nb_subs):
            for sample_id in range(nb_samples):
                ...
                y_min[sub_id]:y_max[sub_id]] = signal
        ...
        for sub_id in range(2):
            for sample_id in range(nb_samples):
                ...
                y_min[sub_id] + bias2:y_max[sub_id] + bias2] = signal
    ...
    for i in range(patterns.shape[0]):
        ...
        plt.imshow(patterns[i], vmin=0, vmax=1.1)
    ...
    fig.suptitle('{}'.format(note), fontsize=14)
    plt.subplot(1, 4, 1)
    plt.imshow(patterns[0], vmin=0, vmax=1.1)
    plt.title('image 1')
    plt.subplot(1, 4, 2)
    plt.imshow(patterns[-1], vmin=0, vmax=1.1)
    plt.title('image 2')
    plt.subplot(1, 4, 3)
